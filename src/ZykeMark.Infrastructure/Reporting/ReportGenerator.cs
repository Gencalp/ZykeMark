...
public sealed class ReportGenerator
{
    ...
    public string Generate(string sessionFolderPath, string? outputPdfPath = null)
    {
        ...
        Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Element(content => BuildContent(content, reportData));
    ...
    private void BuildContent(IContainer container, ReportData data)
    {
    ...
    private void BuildKeyMetrics(IContainer container, ReportData data)
    {
        container.Column(column =>
        {
            ...
            column.Item().Row(row =>
            {
                row.RelativeItem().Text($"Avg FPS: {data.AvgFps.ToString("F1", CultureInfo.InvariantCulture)}");
                row.RelativeItem().Text($"1% Low: {data.OnePercentLowFps.ToString("F1", CultureInfo.InvariantCulture)}");
                row.RelativeItem().Text($"0.1% Low: {data.PointOnePercentLowFps.ToString("F1", CultureInfo.InvariantCulture)}");
            });
            column.Item().Row(row =>
            {
                row.RelativeItem().Text($"Avg Frame Time: {data.AvgFrameTimeMs.ToString("F2", CultureInfo.InvariantCulture)} ms");
                row.RelativeItem().Text($"P99 Frame Time: {data.P99FrameTimeMs.ToString("F2", CultureInfo.InvariantCulture)} ms");
                row.RelativeItem().Text($"Duration: {FormatDuration(data.DurationMs)}");
            });
        });
    }
